    ...
    return (
      ...
      >
        ...
        <div className="w-full h-full flex flex-col overflow-hidden rounded-l-3xl">
          ...
          <form onSubmit={handleFormSubmit} className="p-6 flex flex-col flex-1 min-h-0 overflow-y-auto custom-scrollbar">
             <div className="space-y-4">
                ...
                <input type="text" value={formTitle} onChange={e => setFormTitle(e.target.value)} placeholder="이슈 제목을 입력하세요" className="w-full text-xl font-bold text-gray-900 border-b border-dashed border-gray-200 hover:border-gray-300 focus:border-[#004b8d] outline-none bg-transparent placeholder-gray-300 transition-colors py-1" required />
                ...
                <div className="relative mb-4">
                  ...
                  {isEquipDropdownOpen && (
                    <div 
                      ...
                      className="absolute left-0 right-0 mt-1 max-h-[480px] overflow-y-auto bg-white border border-gray-200 rounded-xl shadow-lg z-50 p-2 custom-scrollbar"
                    >
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 px-1">
                            {group.equipments.map(eq => (
                              <button
                                ...
                                className={`text-left text-xs px-2 py-1 rounded-md transition-colors truncate ${
                                  formEquipName === eq 
                ...
                <textarea value={formContent} onChange={e => setFormContent(e.target.value)} placeholder="자세한 내용이나 증상을 기록합니다..." rows={6} className="w-full bg-gray-50 p-4 rounded-xl text-sm text-gray-700 whitespace-pre-wrap leading-relaxed border border-gray-100 hover:border-gray-200 focus:border-[#004b8d] focus:bg-white outline-none resize-none placeholder-gray-400 transition-all shadow-inner" required />
                
                <div className="flex flex-wrap items-center gap-4 mt-4 text-xs text-gray-500 font-medium bg-gray-50/50 p-3 rounded-xl border border-gray-50">
             ...
             <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-gray-100 shrink-0">
              <button type="button" onClick={closeModal} className="px-5 py-2.5 text-sm font-bold text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all">취소</button>
              <button type="submit" className="px-5 py-2.5 text-sm font-bold text-white bg-[#004b8d] rounded-xl hover:bg-[#003c71] transition-all shadow-md">{modalMode === 'create' ? '새 글 등록' : '수정 완료'}</button>
             </div>
          </form>
        </div>
      </div>
      ...
      >
        ...
        <div className="w-full h-full flex flex-col overflow-hidden rounded-l-3xl">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
            <h3 className="font-bold text-gray-900 text-xl">장비 이슈 상세</h3>
            <div className="flex items-center gap-2">
              <button 
                type="button" 
                onClick={() => setIsPinned(!isPinned)} 
          ...
          <div className="p-5 overflow-y-auto flex-1 space-y-4 custom-scrollbar">
             <div className="space-y-4">
                ...
                <h4 className="text-2xl font-bold text-gray-900 mb-1">{currentIssue.title || currentIssue.equipmentName}</h4>
                ...
                <div className="flex justify-end gap-3 pt-3 mt-2 border-t border-gray-100 shrink-0">
                  <button type="button" onClick={() => setSelectedIssue(null)} className="px-5 py-2.5 text-base font-bold text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all">닫기</button>
                </div>
             </div>
